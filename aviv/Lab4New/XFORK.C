...
if (currpid != pid) /* parent process only */
	...
	*(int *)sp3 = INITF;		/* FLAGS value			*/
    ...
    //Lab 4 change--------------------------------------------------------------------------------------
    pidOfFather[pid] = currpid; // update the pid of the father
    numOfSons[currpid]++; // update the number of the sons
    
    if(son_first[currpid] == 0) //update if this is the first son
      son_first[currpid] = pid; 
    //Lab 4 change--------------------------------------------------------------------------------------
